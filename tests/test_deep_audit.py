...
class TestXLSXExtraction:
    ...
    def test_xlsx_row_cap_at_500(self):
        """Only first 500 data rows are extracted per sheet (ETL cap)."""
        import openpyxl, io as _io
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.append(["Value"])
        ...
        assert len(data_rows) <= 500, f"More than 500 rows extracted: {len(data_rows)}"
        assert len(data_rows) == 500, f"Expected exactly 500 rows, got {len(data_rows)}"
