...
_OCR_PROMPT = (
    ...
    "PAGES: Begin each page's content with a marker [Page N] (e.g. [Page 1], [Page 2]). "
    "TABLES: Preserve table structure using | separators between columns and new lines "
    "between rows. Always output the column header row first, then data rows. "
    "Before each table, output its surrounding heading or context on its own line "
    "(e.g. 'Table: Income Statement (USD million)') so column values can be mapped "
