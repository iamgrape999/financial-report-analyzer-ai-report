...
SECTION_INSTRUCTIONS: dict[int, str] = {
    1: (
        ...
        "Account Strategy. If ANY is missing → flag I-5: FAIL in QA summary. "
        "Do NOT restart, repeat, or re-output any content.\n"
        ...
        "11. ⛔ NO re-outputting, repeating, or duplicating any section or paragraph. "
        "Each segment of §1 appears EXACTLY ONCE. If QA fails, note it — do NOT regenerate.\n\n"
        ...
        "## I. QA Gate (MANDATORY — Execute ONCE, record results only)\n"
        "⛔ ZERO DUPLICATION: Every section of §1 appears EXACTLY ONCE in the output. "
        "Do NOT re-output, repeat, or regenerate any content — even if a check FAILS.\n"
        "Execute checks as you write. If a check FAILS, note it in the QA summary line — "
        "do NOT restart or re-output any section.\n"
        "**APPEND QA summary as a single final line: [QA-I1: PASS/FAIL] ... [QA-I8: PASS/FAIL]**\n\n"
    ...
    2: (
        ...
        "If ANY table is missing → flag FAIL in QA summary. "
        "Do NOT restart, repeat, or re-output any content.\n\n"
        ...
        "## H. QA Gate (MANDATORY — Execute ONCE, record results only)\n"
        "⛔ ZERO DUPLICATION: Each table appears EXACTLY ONCE. "
        "Do NOT re-output, repeat, or regenerate any content — even if a check FAILS.\n"
        "Execute checks as you write. If a check FAILS, mark it FAIL in QA summary — do NOT regenerate.\n"
        "**APPEND QA summary as a single final line: [QA-H1: PASS/FAIL] ... [QA-H7: PASS/FAIL]**\n\n"
    ...
    3: (
        ...
        "(all paragraphs) + ESG are ALL present. If ANY is missing → flag FAIL in QA summary. "
        "Do NOT restart, repeat, or re-output any content.\n"
        "Overflow → split: End: '[§3 CONTINUED IN NEXT OUTPUT]' / Resume: '[§3 CONTINUED]'\n\n"

        "## J. QA Gate (MANDATORY — Execute ONCE, record results only)\n"
        "⛔ ZERO DUPLICATION: Each section appears EXACTLY ONCE. "
        "Do NOT re-output, repeat, or regenerate any content — even if a check FAILS.\n"
        "Execute checks as you write. If a check FAILS, mark it FAIL in QA summary — do NOT regenerate.\n"
        "APPEND QA summary as a single final line: [QA-J1: PASS/FAIL] ... [QA-J8: PASS/FAIL]\n\n"
    ...
    7: (
        ...
        "**FULL STATEMENT OVERRIDE (PRIORITY RULE):** "
        "If `full_income_statement` is present in 7A → output ALL its rows verbatim as the P&L table; "
        "item names may be Chinese/bilingual — translate each to standard English label in the output; "
        "values are exact as given; ✅CHECK auto-satisfied. "
        "Same for `full_balance_sheet` → BS table; `full_cash_flow` → CF table. "
        "DO NOT add rows beyond those provided; DO NOT fill missing cells with N/A.\n"
        "**When full statements absent**, construct tables from income_statement/balance_sheet/cash_flow aggregate fields:\n"
        ...
        "**PBT** → Tax → **Net Income (NM%)** ✅CHECK: ≥12 rows (when full statement provided, count those rows).\n\n"
        ...
        "✅CHECK: ≥20 rows (when full statement provided, count those rows). "
        "Do NOT collapse liabilities to single total.\n\n"
        "**CF items — output ALL:** "
        "OCF | ICF | FCF | **Net Change** | Opening Cash | FX Effect | **Closing Cash** "
        "✅CHECK: ≥7 rows (when full statement provided, count those rows). "
        "If Opening/Closing Cash absent from CF input, derive from BS Cash row across consecutive periods.\n\n"

        "### C-2. Borrower Summary Statistics (MANDATORY)\n"
        "**SOURCE PRIORITY:** Use 7B input values verbatim if present; "
        "otherwise compute from 7A data (income_statement + balance_sheet + cash_flow + full statements). "
        "For full statement inputs (may be in Chinese): map item names to their financial metrics. "
        "Efficiency ratios: derive AR/AP/Inventory from full_balance_sheet rows if available. "
        "OMIT a ratio row only if NEITHER 7B provides it NOR it is calculable from 7A. "
        "NEVER write 'N/A' for annual-period data.\n"
        ...
        "✅CHECK: ≥18 ratio rows for annual data (rows with calculable data only).\n"
        ...
        "**Ratios — COMPUTE or OMIT, never write 'N/A' for annual FY data:** "
        "If 7B input contains a ratio value → USE IT verbatim. "
        "If 7B absent → compute from 7A data (e.g. ROA=NI/AvgTA, ROE=NI/AvgEq, "
        "OCF/Debt=OCF/TotalDebt, OCF/Int=OCF/FinanceCost, "
        "GrossMargin=(Rev-COGS)/Rev, EBITDAMargin=EBITDA/Rev, "
        "OpMargin=OpProfit/Rev, NetDebt=TotalDebt-Cash, D/E=TotalDebt/Equity). "
        "Denominator ≤0 or meaningless → N/M. "
        "For interim periods requiring annualization (ROA,ROE,Debt/EBITDA,EBITDA/Int,OCF/Debt,OCF/Int) → N/M. "
        "Point-in-time (D/E, NetDebt/Eq) → compute from period-end BS normally. "
        "Row where NEITHER provided in 7B NOR calculable from 7A → OMIT the row; NEVER write 'N/A'.\n"
        ...
        "⛔ ZERO DUPLICATION: Each table and section appears EXACTLY ONCE — do NOT re-output any content.\n"
        "After drafting, EXECUTE all gates. If any fails → note FAIL in the QA line only — do NOT re-output any content.\n"
